...
def get_saved_patient_id():
    """Retrieve the saved patient ID from the file."""
    file_path = Path(RESOURCE_FILE)
    if not file_path.exists():
        print(f"Error: {RESOURCE_FILE} file not found.")
        return None
    try:
        with open(file_path, 'r') as file:
            return file.read().strip()
    except Exception as e:
        print(f"Error reading resource ID from file: {e}")
        return None
...
if __name__ == '__main__':
    ...
    if patient_id:
        ...
        if patient_details:
            print(f"Patient Details:\n{json.dumps(patient_details, indent=4)}")

            saved_patient_id = get_saved_patient_id()
            if saved_patient_id:
                print(f"Using saved patient ID for posting condition: {saved_patient_id}")
                condition_code = get_one_condition(patient_resource_id=patient_id)

                if condition_code:
                    child_snomed_code, child_snomed_display = get_direct_child_snomed(condition_code)
                    if child_snomed_code and child_snomed_display:
                        print("\nGenerating and posting condition data for direct child...")
                        condition_json = generate_condition_json(
                            saved_patient_id, child_snomed_code, child_snomed_display
                        )
                        post_condition_to_primary_care(condition_json)
            else:
                print("No saved patient ID found. Unable to post condition.")